    ...
    if Old_World then
        if SelectTpSpot == "Pirate Starter" then
            if InstanceTp then
                ...
                if (CFrameTargetTp.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude > 6000 then
        ...
        elseif SelectTpSpot == "Skyland 4th" then
            if InstanceTp then
                ...
                if (CFrameTargetTp.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude > 3000 then
                    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(-4607.82275390625, 874.3905029296875, -1667.556884765625))
                end
            end
...
local function CheckLevel()
    ...
    if Old_World then
        ...
        elseif Lv == 200 or Lv <= 249 or SelectMonster == "Dark Master [Lv. 175]" then -- Dark Master
            Ms = "Dark Master [Lv. 175]"
            NameQuest = "SkyQuest"
            QuestLv = 2
            NameMon = "Dark Master"
