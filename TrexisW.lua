...
local CFrameQ = nil
local CFrameMon = nil
_G.AuraMaterialType1 = false
_G.AutoMaterial = false
_G.AutoFarmLevel = false
_G.CustomQuest = false
Old_World = false
New_World = false
Third_World = false
...
while _G.AutoFarmLevel do
    ...
    repeat
        for _,v in pairs(workspace.Enemies:GetChildren()) do
            if v and v.Name == NameMon and v:FindFirstChild("Humanoid") and v.Humanoid.Health ~= 0 and _G.AutoFarmLevel then
                local MobHumP = v.HumanoidRootPart.Position
                Attack = true
                repeat
                tpwithnewtpbyme(MobHumP.X,MobHumP.Y + 50,MobHumP.Z, 10)
                wait()
                until v.Humanoid.Health == 0 or game.Players.LocalPlayer.PlayerGui.Main.Quest.Visible == false or not _G.AutoFarmLevel
            end
        end
        tpwithnewtpbyme2(CFrameMon, 10)
        task.wait()
    until game.Players.LocalPlayer.PlayerGui.Main.Quest.Visible == false or not _G.AutoFarmLevel
    Attack = false
    task.wait()
end
while _G.AuraMaterialType1 and _G.AutoMaterial do
    float = true
	if _G.CustomQuest then
		tpwithnewtpbyme2(CFrameQ, 5)
		task.wait(1)
		game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("StartQuest", NameQuest, QuestLv)
		task.wait(3)
    ...
    repeat
        for _,v in pairs(workspace.Enemies:GetChildren()) do
            if v and v.Name == NameMon and v:FindFirstChild("Humanoid") and v.Humanoid.Health ~= 0 and (v.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude > 3000 and _G.AuraMaterialType1 and _G.AutoMaterial then
                local MobHumP = v.HumanoidRootPart.Position
                Attack = true
                repeat
                tpwithnewtpbyme(MobHumP.X,MobHumP.Y + 50,MobHumP.Z, 10)
...
local LeftGroupBox = Tabs.Main:AddLeftGroupbox('Auto Farm Level')
...
local LeftGroupBox = Tabs.Main:AddLeftGroupbox('Auto Farm Material (Sea 1)')
LeftGroupBox:AddDropdown('Select Material', {
    Values = { 'Angles Wings', 'Scrap Metal', 'Leather', 'Magma Ore', 'Yeti Fur' },
    Default = nil,
    Multi = false,
    ...
    Callback = function(Value)
        if Value == 'Angles Wings' then
			...
			CFrameQ = CFrame.new(-4721.8603515625, 845.30297851563, -1953.8489990234)
			CFrameMon = CFrame.new(-4628.0498046875, 866.92877197266, -1931.2352294922)
		elseif Value == 'Scrap Metal' then
			_G.AuraMaterialType1 = true
			Ms = "Brute [Lv. 45]"
			NameQuest = "BuggyQuest1"
			QuestLv = 2
			NameMon = "Brute"
...
local LeftGroupBox = Tabs.Main:AddLeftGroupbox('Auto Farm Material (Sea 2)')
LeftGroupBox:AddDropdown('Select Material', {
    Values = { 'Leather', 'Radioactive Material', 'Scrap Metal', 'Vampire Fang', 'Magma Ore', 'Mystic Droplet' },
    Default = nil,
    Multi = false,
    ...
    Callback = function(Value)
        if Value == 'Leather' then
			...
			CFrameQ = CFrame.new(-6061.841796875, 15.926671981812, -4902.0385742188)
			CFrameMon = CFrame.new(-5720.5576171875, 63.309471130371, -4784.6103515625)
		elseif Value == 'Radioactive Material' then
			_G.AuraMaterialType1 = false
			Ms = "Factory Staff [Lv. 800]"
			NameQuest = "Area2Quest"
			QuestLv = 2
		...
		elseif Value == 'Scrap Metal' then
			...
			CFrameQ = CFrame.new(-6061.841796875, 15.926671981812, -4902.0385742188)
			CFrameMon = CFrame.new(-5720.5576171875, 63.309471130371, -4784.6103515625)
		elseif Value == 'Vampire Fang' then
			_G.AuraMaterialType1 = false
			Ms = "Vampire [Lv. 975]"
			NameQuest = "ZombieQuest"
			QuestLv = 2
		...
		elseif Value == 'Magma Ore' then
			...
			CFrameQ = CFrame.new(-5429.0473632813, 15.977565765381, -5297.9614257813)
			CFrameMon = CFrame.new(-5461.8388671875, 130.36347961426, -5836.4702148438)
		elseif Value == 'Mystic Droplet' then
			_G.AuraMaterialType1 = true
			Ms = "Sea Soldier [Lv. 1425]"
			NameQuest = "ForgottenQuest"
			QuestLv = 1
...
local LeftGroupBox = Tabs.Main:AddLeftGroupbox('Auto Farm Material (Sea 3)')
LeftGroupBox:AddDropdown('Select Material', {
    Values = { '', '', '', '', '', '' },
    Default = nil,
    Multi = false,
